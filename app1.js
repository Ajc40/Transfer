...
and serve the game files. It runs plain HTTP so it can be accessed
from the outward-facing IP 104.241.54.1 on the VM.
*/
...
Note: Safari requires HTTPS for Brotli. Chrome/Edge/Firefox work over HTTP.
*/
...
secure: false because we are running plain HTTP on the VM.
*/
...
/*
Starts a plain HTTP server bound to all interfaces (0.0.0.0)
so anyone hitting the outward-facing IP 104.241.54.1 can reach the app.
Make sure port 3000 (or $PORT) is open in your VM firewall/security group.
*/
async function startServer() {
    const host = "0.0.0.0";
    const publicIp = process.env.PUBLIC_IP || "104.241.54.1";

    app.listen(port, host, () => {
        console.log(`Server started on http://${publicIp}:${port}`);
        console.log(`Listening on all interfaces (${host}:${port})`);
        console.log("Note: Brotli Unity assets work in Chrome/Edge/Firefox over HTTP. Safari requires HTTPS.");
    });
}
